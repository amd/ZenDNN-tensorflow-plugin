...
function prepare_src() {
  ...
  if [ -d bazel-bin/tensorflow_plugin/tools/pip_package/build_pip_package.runfiles/org_tensorflow_plugin/external ]; then
    ...
    cp_external bazel-bin/tensorflow_plugin/tools/pip_package/build_pip_package.runfiles/org_tensorflow_plugin/external "${EXTERNAL_INCLUDES}"
    # Copy ZenDNN libs over so they can be loaded at runtime.
    so_lib_dir=$(ls $RUNFILES | grep solib) || true
    if [ -n "${so_lib_dir}" ]; then
      plugin_so_dir=$(ls ${RUNFILES}/${so_lib_dir} | grep zen) || true
      if [ -n "${plugin_so_dir}" ]; then
        mkdir "${TMPDIR}/${so_lib_dir}"
        for dir in ${plugin_so_dir}
        do
          cp -R -d ${RUNFILES}/${so_lib_dir}/${dir} "${TMPDIR}/${so_lib_dir}"
        done
      fi
    fi
  else
