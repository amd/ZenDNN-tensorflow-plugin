...
bool IsL2Loss(const NodeDef& node);
...
bool IsApplyAdam(const NodeDef& node);
...
bool IsDequantize(const NodeDef& node);
...
bool IsQuantizeV2(const NodeDef& node);
...
#endif  // TENSORFLOW_PLUGIN_SRC_AMD_CPU_GRAPH_UTILS_OP_TYPES_H_
