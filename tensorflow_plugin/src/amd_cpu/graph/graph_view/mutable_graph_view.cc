...
string GeneratedNameForIdentityConsumingSwitch(
  ...
  return AddPrefixToNodeName(
      strings::StrCat(fanin.node->name(), "_", fanin.port_id),
...
void MutableGraphView::AddAndDedupFanouts(NodeDef* node) {
  // TODO(plugin): Checks for self loops, Switch control dependencies, fanins
...
Status MutableGraphView::DeleteNodes(
  ...
  // TODO(plugin): Use a node name->idx hashmap if this is a performance
  // bottleneck.
  int pos = 0;
