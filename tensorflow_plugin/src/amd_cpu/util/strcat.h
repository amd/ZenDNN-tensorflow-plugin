...
//
#ifndef TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_STRCAT_H_
#define TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_STRCAT_H_

#include <string>

#include "tensorflow_plugin/src/amd_cpu/util/macros.h"
#include "tensorflow_plugin/src/amd_cpu/util/numbers.h"
...
class AlphaNum {
 public:
  ...
  AlphaNum(long long int i64)  // NOLINT
      : piece_(digits_, FastInt64ToBufferLeft(i64, digits_)) {}
  AlphaNum(unsigned long long int u64)  // NOLINT
  ...
  AlphaNum(Eigen::bfloat16 bf)  // NOLINT(runtime/explicit)
  ...
  AlphaNum(const amd_cpu_plugin::string& str)      // NOLINT(runtime/explicit)
      : piece_(str) {}
  AlphaNum(const amd_cpu_plugin::tstring& str)  // NOLINT(runtime/explicit)
      : piece_(str) {}
  // clang-format off
  template <typename A>
  AlphaNum(const std::basic_string<char, std::char_traits<char>, A>& str)  // NOLINT
      : piece_(str) {}
  // clang-format on
...
#endif  // TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_STRCAT_H_
