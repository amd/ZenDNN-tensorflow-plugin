...
#ifndef TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_STACKTRACE_H_
#define TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_STACKTRACE_H_
...
#include <sstream>
#include <string>
#include "tensorflow_plugin/src/amd_cpu/util/abi.h"
...
inline std::string CurrentStackTrace() {
  ...
  for (int i = 0; i < buffer_size; ++i) {
    ...
    std::string demangled = amd_cpu_plugin::port::MaybeAbiDemangle(symbol);
    if (demangled.length()) {
...
#endif  // TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_STACKTRACE_H_
