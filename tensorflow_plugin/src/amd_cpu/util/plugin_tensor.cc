...
#include "tensorflow_plugin/src/amd_cpu/util/tensor_coding.h"
#include "tensorflow_plugin/src/amd_cpu/util/tensor_shape.h"
#include "tensorflow_plugin/src/amd_cpu/util/type_traits.h"
...
void Tensor::AsProtoTensorContent(TensorProto* proto) {
  ...
  if (buf_) {
    ...
    tmp_buf_ = new amd_cpu_plugin::TensorBuffer(
        reinterpret_cast<void*>(TF_TensorData(buf_)));
