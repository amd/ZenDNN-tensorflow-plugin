...
#ifndef TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_PLUGIN_TENSOR_H_
#define TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_PLUGIN_TENSOR_H_
...
class Tensor {
 public:
  ...
  void set_shape(const TensorShape& shape) {
    DataType dt = dtype();
    TF_SetShape(buf_, shape.dim_sizes().data(), shape.dims());
    shape_ = shape;
    set_dtype(dt);
  }
...
#endif  // TENSORFLOW_PLUGIN_SRC_AMD_CPU_UTIL_PLUGIN_TENSOR_H_
