...
static bool ConsumeJobName(StringPiece* in, std::string* job) {
  return ConsumePrefix(in, job, "/");
}

// Returns true and fills in "*device_type" iff "*in" starts with a device type
// name.
static bool ConsumeDeviceType(StringPiece* in, std::string* device_type) {
...
//                          const string& device_prefix, const string&
//                          device_type, int id) {
...
// namespace {
// string LegacyName(const string& job, int replica, int task, const string&
// type,
//                   int id) {
//   return DeviceName(job, replica, task, "/", absl::AsciiStrToLower(type),
//   id);
// }
// }  // anonymous namespace
...
/* static */
std::string DeviceNameUtils::LocalName(StringPiece type, int id) {
...
// '/device:type:id').
std::string LegacyLocalName(StringPiece type, int id) {
  return strings::StrCat(type, ":", id);
}
}  // anonymous namespace

/* static */
std::string DeviceNameUtils::LocalName(StringPiece fullname) {
  ParsedName x;
...
/* static */
bool DeviceNameUtils::SplitDeviceName(StringPiece name, std::string* task,
                                      std::string* device) {
  ParsedName pn;
...
//     const ParsedName& pn) {
//   if (pn.has_job && pn.has_replica && pn.has_task && pn.has_type &&
//   pn.has_id) {
//     return {
//         DeviceNameUtils::FullName(pn.job, pn.replica, pn.task, pn.type,
//         pn.id), LegacyName(pn.job, pn.replica, pn.task, pn.type, pn.id)};
//   } else {
//     return {};
//   }
// }
