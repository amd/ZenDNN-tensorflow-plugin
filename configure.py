...
def setup_python(environ_cp):
  ...
  with open(
      os.path.join(_TF_WORKSPACE_ROOT, 'tensorflow_plugin', 'tools',
...
def get_var(environ_cp,
  ...
  """
  if not question:
    question = ('Do you wish to build TensorFlow plug-in'
                ' with %s support?') % query_item
  if not yes_reply:
    yes_reply = ('%s support will be enabled for '
                 'TensorFlow plug-in.') % query_item
  if not no_reply:
...
def set_gcc_host_compiler_path(environ_cp):
  ...
  default_gcc = which('gcc')
  if os.path.islink(default_gcc):
    # os.readlink is only available in linux
    default_gcc_host_compiler_path = os.path.realpath(default_gcc)

  gcc_host_compiler_path = prompt_loop_or_load_from_env(
      environ_cp,
      var_name='GCC_HOST_COMPILER_PATH',
      var_default=default_gcc_host_compiler_path,
      ask_for_var=('Please specify which gcc should be used by nvcc'
...
def set_mpi_home(environ_cp):
  ...
  def valid_mpi_path(mpi_home):
    ...
    if not exists:
      print('Invalid path to the MPI. %s or %s or %s or %s cannot be found'
